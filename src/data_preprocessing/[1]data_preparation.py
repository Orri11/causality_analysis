...
data_train_adj.to_csv(os.path.join(data_path,'elec_price_dataset.txt'), index= False, header=False)


# Adjust test set to right format for forecasting task
data_test.to_csv(os.path.join(data_path,'elec_price_results.txt'), index = True, header = False)